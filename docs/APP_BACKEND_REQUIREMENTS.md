...
### 3-3. ⚠️ iOS 는 토큰 종류가 다릅니다 — 결정 필요

안드로이드는 FCM 토큰이 나오지만, **iOS 는 현재 구성에서 APNs 토큰이 나옵니다.**
`POST /device-tokens` 의 `platform` 값으로 구분할 수 있습니다.

선택지는 둘입니다.

**(A) 서버가 FCM + APNs 둘 다 구현** — 프론트 변경 없음
- `platform: "android"` → FCM 으로 전송
- `platform: "ios"` → APNs 로 직접 전송 (Apple 에서 발급한 `.p8` 키 + JWT 인증)
- 서버가 전송 채널을 두 개 관리해야 합니다.

**(B) iOS 도 FCM 토큰을 쓰도록 앱을 변경** — 권장
- 앱에 Firebase iOS SDK 를 넣어 APNs 토큰을 FCM 토큰으로 교환합니다.
- 서버는 **FCM 하나만** 구현하면 됩니다. 위에 적은 내용 그대로입니다.
- Apple 에서 발급한 APNs 키를 Firebase 콘솔에 한 번 업로드하는 절차가 추가됩니다.
  (Apple Developer 계정이 있어야 가능해서 지금은 못 합니다.)

**(B) 를 권장합니다.** 서버 작업량이 절반이고, 발송 로직·실패 처리·통계가 한 곳으로 모입니다.
어느 쪽으로 갈지 정해주시면 프론트를 맞추겠습니다.

당장은 **안드로이드부터 출시**할 예정이라 이 결정이 급하진 않습니다.
다만 (A) 로 간다면 서버 설계 초기에 알고 있어야 해서 미리 적어둡니다.
