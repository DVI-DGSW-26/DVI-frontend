...
### 3-3. iOS 도 FCM 으로 통일합니다 — 서버 추가 작업 없음

iOS 는 원래 APNs 토큰이 나오지만, **앱에 Firebase iOS SDK 를 넣어 FCM 토큰으로
통일하기로 했습니다.** 프론트에서 처리하는 부분입니다.

따라서 **서버 입장에서는 안드로이드와 iOS 가 완전히 동일합니다.**
위 3-1 / 3-2 에 적은 FCM 하나만 구현하시면 됩니다. 전송 채널을 두 개
관리하실 필요 없습니다.

`platform` 값(`"android"` / `"ios"` / `"web"`)은 계속 저장해 주세요.
전송 방식이 같아도 통계·디버깅에 필요하고, 나중에 플랫폼별로 문구를
다르게 보낼 여지도 남습니다.

> Apple APNs 키를 Firebase 콘솔에 업로드하는 절차가 있는데, 이건 앱 쪽
> 작업이라 서버와 무관합니다. Apple Developer 계정 등록 후에 진행합니다.
